...
void Map::RemoveFromMap(T *obj, bool remove)
{
    ...
    if constexpr (std::is_base_of_v<Creature, T>)
    {
        if (obj->IsNPCBot())
        {
            ...
            {
                Creature const* bot = obj->ToCreature();
                bot_ai const* ai = bot->GetBotAI();
                bool const expectedNoGrid = !ai || ai->IsDuringTeleport() || bot->IsWandererBot() || ai->IsTempBot() ||
                    bot->GetEntry() == BOT_ENTRY_MIRROR_IMAGE_BM;

                if (!expectedNoGrid)
                {
                    Player const* owner = bot->GetBotOwner();
                    BOT_LOG_WARN("npcbots",
                        "[Map] 移除Bot不在格子内: {} ({}) 地图={}/{} 实例={} | 主人={} 主人地图={}/{}",
                        bot->GetName(), bot->GetEntry(), GetId(), GetMapName(), i_InstanceId,
                        owner ? owner->GetGUID().ToString() : std::string("none"),
                        (owner && owner->GetGUID().IsPlayer() && owner->IsInWorld()) ? owner->GetMapId() : 0u,
                        (owner && owner->GetGUID().IsPlayer() && owner->IsInWorld()) ? owner->GetMap()->GetMapName() : "none");
                }
            }
