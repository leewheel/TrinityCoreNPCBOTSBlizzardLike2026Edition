...
void BotMgr::_teleportBot(Creature* bot, Map* newMap, float x, float y, float z, float ori, bool quick, bool reset, bot_ai* detached_ai)
{
    ...
    BotMgr::AddDelayedTeleportCallback([bot, botai, newMap, x, y, z, ori, quick, reset]() {
        ...
        if (mymap)
        {
            ...
            // NpcBot RemoveFromMap handles bots already dropped from grid (wanderer TP / mirror images)
            if (bot->IsNPCBot() || bot->IsInGrid())
...
void BotMgr::SetRandomBotTalentsForGroup(Creature const* bot, uint32 botrole)
{
    ...
    uint32 disableMask = BOT_ROLE_TANK | BOT_ROLE_TANK_OFF | BOT_ROLE_DPS | BOT_ROLE_HEAL | BOT_ROLE_RANGED;
    if (ai->HasRole(disableMask))
        ai->ToggleRole(disableMask, true);

    uint32 enableMask = BOT_ROLE_DPS;
    if (botrole == BOT_ROLE_TANK)
        enableMask = BOT_ROLE_TANK | BOT_ROLE_DPS;
    else if (botrole == BOT_ROLE_TANK_OFF)
        enableMask = BOT_ROLE_TANK | BOT_ROLE_TANK_OFF | BOT_ROLE_DPS;
    else if (botrole == BOT_ROLE_HEAL)
        enableMask = BOT_ROLE_HEAL | BOT_ROLE_RANGED;
    else if (botrole == BOT_ROLE_DPS)
        enableMask = BOT_ROLE_DPS;

    if (!ai->HasRole(enableMask))
        ai->ToggleRole(enableMask, true);
...
BotAddResult BotMgr::AddServiceBot(Creature* bot, uint32 botRole)
{
    ...
    // Pull bot to the player from anywhere in the world (not tied to recruiter NPC).
    if (!bot->IsInWorld() || bot->GetMapId() != _owner->GetMapId() || !bot->IsWithinDist(_owner, 50.0f))
        TeleportBot(bot, _owner->GetMap(), _owner);
