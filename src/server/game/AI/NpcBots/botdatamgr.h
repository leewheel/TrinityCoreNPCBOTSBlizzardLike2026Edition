...
struct NpcBotData
{
    ...
    NpcBotData(uint32 bowner, uint64 bhire_time, uint32 broles, uint32 bfaction, uint8 bspec, uint8 bhire_source = NPCBOT_HIRE_NORMAL)
        : owner(bowner), hire_time(bhire_time), roles(broles), faction(bfaction), spec(bspec), hire_source(bhire_source) {}
...
struct NpcBotAppearanceData
{
    ...
    NpcBotAppearanceData(std::string bname, uint8 bgender, uint8 bskin, uint8 bface, uint8 bhair, uint8 bhaircolor, uint8 bfeatures)
        : name(std::move(bname)), gender(bgender), skin(bskin), face(bface), hair(bhair), haircolor(bhaircolor), features(bfeatures) {}
...
public:
    ...
    static bool UsesGeneratedBotRangedSlot(uint8 botclass);
    static uint32 GetWandererHeirloomWeaponFallback(uint8 botclass, uint8 slot);
    static Item* TryCreateWanderingBotEquipItem(uint32 entry);
    ...
    static Item* GenerateWanderingBotRelicItem(uint8 category, uint8 botclass, uint8 level, uint32 maxitemlevel);
    ...
    static uint32 TextForClass(uint8 botclass);
