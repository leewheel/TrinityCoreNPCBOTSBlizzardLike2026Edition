...
SpellCastResult bot_ai::CheckBotCast(Unit const* victim, uint32 spellId) const
{
    ...
    {
        bool const canTargetDead = (spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_DEAD) ||
        ...
        if (!canTargetDead && victim->getDeathState() != DeathState::JUST_DIED)
            return SPELL_FAILED_BAD_TARGETS;
    }
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    if (victim->isType(TYPEMASK_UNIT) && victim->isDead())
    {
        bool const canTargetDead = (m_botSpellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_DEAD) ||
        ...
        if (!canTargetDead)
        {
            if (victim->getDeathState() == DeathState::JUST_DIED)
                return false;

            Unit* const curVictim = me->GetVictim();
            if (curVictim && curVictim->GetGUID() == victim->GetGUID())
                me->AttackStop();
    ...
    if (IsWanderer())
    {
        if (_baseLevel == 0) //this only happens once
        {
            ...
            if (me->GetMap()->IsBattlegroundOrArena())
                ...
                WANDER_BOT_LOG_DEBUG("npcbots", "Wandering bot {} id {} selected level {}...", me->GetName(), me->GetEntry(), uint32(_baseLevel));
        }
...
std::pair<Unit*, Unit*> bot_ai::_getTargets(bool byspell, bool ranged, bool &reset) const
{
    ...
    if (mytar && mytar->isType(TYPEMASK_UNIT) && !mytar->IsAlive())
        return { nullptr, nullptr };
...
bool bot_ai::_isItemFitForGeneratedBot([[maybe_unused]] uint8 category, uint8 slot, ItemTemplate const* proto) const
{
    ...
    // libram/totem/idol/sigil — only need to be equippable in ranged slot, not spec primary stats
    if (slot == BOT_SLOT_RANGED && proto->Class == ITEM_CLASS_ARMOR &&
...
{
    uint8 spec;
    if (IsWanderer() && BotDataMgr::IsValidSpecForClass(_botclass, _botData->spec))
        spec = _botData->spec;
    else if (IAmFree() && !me->IsSummon())
...
void bot_ai::InitRandomEquipWithQuality()
{
    ...
    auto fit_check = [gen_category, lvl, this](uint8 slot, ItemTemplate const* proto) {
        if (!_isItemFitForGeneratedBot(gen_category, slot, proto))
            return false;
...
void bot_ai::DestroyServiceEquips()
{
    ...
    BotDataMgr::UpdateNpcBotData(me->GetEntry(), NPCBOT_UPDATE_EQUIPS, _equips.data());
    BotDataMgr::ResetNpcBotTransmogData(me->GetEntry(), false);
}
...
void bot_ai::InitEquips()
{
    ...
    if (is_wanderer || me->IsSummon())
    {
        ...
        GenerateRand();
        std::string const classSpecLabel = fmt::format("{}/{}",
            LocalizedNpcText(nullptr, BotDataMgr::TextForClass(_botclass)),
            LocalizedNpcText(nullptr, BotDataMgr::TextForSpec(_spec)));
        ...
        for (auto i : NPCBots::index_array<uint8, BOT_INVENTORY_SIZE>)
        {
            if (i == BOT_SLOT_OFFHAND && (!_canUseOffHand() || (lvl < 10 && BotDataMgr::IsCastingClass(_botclass))))
                continue;
            if (i == BOT_SLOT_RANGED && !BotDataMgr::UsesGeneratedBotRangedSlot(_botclass))
                continue;
            if ((i == BOT_SLOT_FINGER1 || i == BOT_SLOT_FINGER2 || i == BOT_SLOT_NECK || i == BOT_SLOT_SHOULDERS) && lvl < 20)
                continue;
            if ((i == BOT_SLOT_TRINKET1 || i == BOT_SLOT_TRINKET2 || i == BOT_SLOT_HEAD) && lvl < 30)
                continue;
            if (_botclass == BOT_CLASS_SPHYNX && i >= BOT_SLOT_BACK)
                continue;

            Item* item = nullptr;
            if (i == BOT_SLOT_RANGED && _canUseRelic())
                item = BotDataMgr::GenerateWanderingBotRelicItem(gen_category, _botclass, lvl, max_item_level);
            if (!item)
                item = BotDataMgr::GenerateWanderingBotItem(gen_category, i, _botclass, lvl, max_item_level, fit_check);
            if (!item && i <= BOT_SLOT_RANGED)
            {
                // spec filters reject valid weapons/relics; retry with equip-only check
                auto const pool_only_check = [this](uint8 s, ItemTemplate const* p) { return _canEquip(p, s, true); };
                item = BotDataMgr::GenerateWanderingBotItem(gen_category, i, _botclass, lvl, max_item_level, pool_only_check);
            }
            if (!item)
            {
                ...
                if (i <= BOT_SLOT_RANGED)
                {
                    if (!(i == BOT_SLOT_RANGED && _canUseRelic()))
                    {
                        fallbackEntry = BotDataMgr::GetWandererHeirloomWeaponFallback(_botclass, i);
                        if (!fallbackEntry)
                            fallbackEntry = einfo->ItemEntry[i];
                    }
                }
                if (fallbackEntry)
                {
                    item = BotDataMgr::TryCreateWanderingBotEquipItem(fallbackEntry);
                    if (item)
                    {
                        _equips[i] = item;
                        BOT_LOG_DEBUG("npcbots",
                            "[装备] 游荡Bot {} ({}) {} L{} 槽位{} 池内无匹配 → 回退 {}",
                            me->GetName(), me->GetEntry(), classSpecLabel, uint32(lvl), uint32(i), fallbackEntry);
                    }
                    else
                    {
                        BOT_LOG_ERROR("npcbots",
                            "[装备] 游荡Bot {} ({}) {} L{} 槽位{} 回退物品 {} 不在 item_template，已跳过",
                            me->GetName(), me->GetEntry(), classSpecLabel, uint32(lvl), uint32(i), fallbackEntry);
                    }
                }
                else
                {
                    BOT_LOG_DEBUG("npcbots",
                        "[装备] 游荡Bot {} ({}) {} L{} 槽位{} 池内无匹配（专精/等级过滤后为空）且无回退",
                        me->GetName(), me->GetEntry(), classSpecLabel, uint32(lvl), uint32(i));
                }
            }
            else
            {
...
void bot_ai::JustDied(Unit* u)
{
    ...
    {
        if (!IsWanderer())
            BOT_LOG_DEBUG("npcbots", "Bot {} id {} class {} level {} WAS KILLED BY {} {} id {} class {} level {}!",
                me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()),
                (u->IsPlayer() ? "player" : u->IsNPCBot() ? "bot" : u->IsNPCBotPet() ? "botpet" : "creature"),
                u->GetName(), u->GetEntry(), uint32(u->GetClass()), uint32(u->GetLevel()));
        else
            WANDER_BOT_LOG_DEBUG("npcbots", "Wandering bot {} id {} class {} level {} WAS KILLED BY {} {} id {} class {} level {} on their way to {}!",
                me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()),
                (u->IsPlayer() ? "player" : u->IsNPCBot() ? u->ToCreature()->GetBotAI()->IsWanderer() ? "wandering bot" : "bot" : u->IsNPCBotPet() ? "botpet" : "creature"),
                u->GetName(), u->GetEntry(), uint32(u->GetClass()), uint32(u->GetLevel()),
                _travel_node_cur ? _travel_node_cur->GetName() : "''");
    }
...
void bot_ai::KilledUnit(Unit* u)
{
    ...
    if (u->IsControlledByPlayer() || u->IsPvP() || u->IsNPCBotOrPet())
    {
        ...
        if (!me->GetMap()->IsBattlegroundOrArena())
        {
            if (IsWanderer())
            {
                WANDER_BOT_LOG_DEBUG("npcbots", "Wandering bot {} id {} class {} level {} KILLED {} {} id {} class {} level {} on their way to {}!",
                    me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()),
                    (u->IsPlayer() ? "player" : u->IsNPCBot() ? u->ToCreature()->GetBotAI()->IsWanderer() ? "wandering bot" : "bot" : u->IsNPCBotPet() ? "botpet" : "creature"),
                    u->GetName(), u->GetEntry(), uint32(u->GetClass()), uint32(u->GetLevel()),
                    _travel_node_cur ? _travel_node_cur->GetName() : "''");
            ...
            {
                WANDER_BOT_LOG_DEBUG("npcbots", "Bot {} id {} class {} level {} KILLED wandering bot {} id {} class {} level {} on their way to {}!",
                    me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()),
                    u->GetName(), u->GetEntry(), uint32(u->GetClass()), uint32(u->GetLevel()),
                    IsWanderer() && _travel_node_cur ? _travel_node_cur->GetName() : "''");
            }
        }
    }
        ...
        if (IAmFree())
        {
            ...
            if (IsWanderer())
            {
                ...
                if (me->GetMap()->GetEntry()->IsContinent() || me->GetMap()->IsBattleground())
                {
                    ...
                    if (!nextNode)
                    {
                        BOT_WP_LOG_FATAL("scripts", "Bot {} ({}) is unable to get next travel node (1)! cur {}, last {}, position: {}",
                            me->GetName(), me->GetEntry(), _travel_node_cur->GetWPId(), _travel_node_last ? _travel_node_last->GetWPId() : 0, me->GetPosition().ToString());
                        BotDataMgr::DisableAndDespawnWanderer(me, "no travel node after death");
                        return;
                    }
                    ...
                    WANDER_BOT_LOG_TRACE("npcbots", "Bot {} id {} class {} level {} died on the way from node {} to {} ('{}'), NEW {} ('{}'), {}, dist {} yd!",
...
void bot_ai::Evade()
{
    ...
    if (IsWanderer())
    {
        // Wrong continent map: teleport directly instead of walking/hearthstone recovery.
        if (_travel_node_cur && mapid != me->GetMap()->GetId() && me->GetMap()->GetEntry()->IsContinent())
        {
            BotMgr::TeleportBot(me, sMapMgr->CreateBaseMap(mapid), _travel_node_cur, true);
            _evadeCount = 0;
            ++_wanderDiagTeleportRecover;
            evadeDelayTimer = 3000;
            return;
        }

        if (_evadeCount >= 50 || me->GetExactDist2d(pos) > MAX_WANDER_NODE_DISTANCE ||
        ...
        if (!me->isMoving())
        {
            ...
            if (dist > (me->GetMap()->GetEntry()->IsContinent() ? 15.0f : INTERACTION_DISTANCE))
            {
                ...
                else
                {
                    ...
                    {
                        BOT_WP_LOG_ERROR("npcbots", "Bot {} '{}' class {} level {} evade move point has invalid height {} (usepath: {})!\nWPs: cur {}, last {}\nPositions:\ncurrent: {}\ntarget: {}",
            ...
            else if (IsWanderer())
            {
                ...
                if (!nextNode)
                {
                    BOT_WP_LOG_FATAL("npcbots", "Bot {} ({}) is unable to get next travel node! cur {}, last {}, position: {}",
                        me->GetName(), me->GetEntry(), _travel_node_cur->GetWPId(), _travel_node_last ? _travel_node_last->GetWPId() : 0, me->GetPosition().ToString());
                    BotDataMgr::DisableAndDespawnWanderer(me, "no travel node on evade");
                    return;
                }
                ...
                else if (nextNode->GetMapId() != me->GetMapId() && me->GetMap()->GetEntry()->IsContinent())
                {
                    BotMgr::TeleportBot(me, sMapMgr->CreateBaseMap(nextNode->GetMapId()), nextNode, true);
                    _travel_node_last = _travel_node_cur;
                    _travel_node_cur = nextNode;
                ...
                WANDER_BOT_LOG_TRACE("npcbots", "Bot {} id {} class {} level {} wandered from node {} to {}, next {} ('{}'), {}, dist {} yd!",
                    me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()), _travel_node_last ? _travel_node_last->GetWPId() : 0, _travel_node_cur->GetWPId(),
...
void bot_ai::GetNextEvadeMovePoint(Position& pos, bool& use_path) const
{
    ...
    if (me->IsInWater() && IsWanderer())
        WANDER_BOT_LOG_TRACE("npcbots", "Bot {} id {} class {} level {} is pathing from water!", me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()));
...
WanderNode const* bot_ai::GetNextWanderNode(Position const* fromPos, uint8 lvl, bool random) const
{
    ...
    {
        if (wpl.wp->GetMapId() != _travel_node_cur->GetMapId())
            continue;
...
namespace
{
std::string WanderLogTruncate(std::string_view text, std::size_t maxBytes)
{
    std::string out(text);
    ...
    while (!out.empty() && (static_cast<unsigned char>(out.back()) & 0xC0) == 0x80)
        out.pop_back();
    if (out.size() + 3 <= maxBytes)
        out.append("...");
    return out;
}

std::string WanderLogPad(std::string_view text, std::size_t width)
{
    std::string cell = WanderLogTruncate(text, width);
    if (cell.size() < width)
        cell.append(width - cell.size(), ' ');
    return cell;
}

std::string WanderLogTargetName(Unit const* unit)
{
    if (!unit)
        return "none";
    ...
    {
        LocaleConstant const loc = sObjectMgr->GetDBCLocaleIndex();
        if (CreatureLocale const* creatureInfo = sObjectMgr->GetCreatureLocale(creature->GetEntry()))
        {
            if (creatureInfo->Name.size() > loc && !creatureInfo->Name[loc].empty())
                return creatureInfo->Name[loc];
        }
...
constexpr char WANDER_SAMPLE_ROW_FMT[] =
    "  {:14} {:>6} {:>4} {:>4} {:>4} {:^8} {:>6} {:>5} {:>3} {:>3} {:18} {:18}";
constexpr char WANDER_SAMPLE_RULE[] =
    "  -------------- ------ ---- ---- ---- -------- ------ ----- --- --- ------------------ ------------------";
}

void bot_ai::LogWandererBehaviorSampleHeader()
{
    BOT_LOG_INFO("npcbots", WANDER_SAMPLE_ROW_FMT,
        "名称", "entry", "职业", "等级", "地图", "状态", "血量", "路点", "PF", "TP", "节点名", "目标");
    BOT_LOG_INFO("npcbots", "{}", WANDER_SAMPLE_RULE);
}

void bot_ai::LogWandererBehaviorSample() const
{
    if (!IsWanderer())
        return;
    ...
    Unit const* victim = me->GetVictim();
    uint32 const nodeId = _travel_node_cur ? _travel_node_cur->GetWPId() : 0;
    std::string_view const nodeName = _travel_node_cur ? std::string_view(_travel_node_cur->GetName()) : std::string_view("none");
    std::string const targetName = WanderLogTargetName(victim);

    uint16 const pathFallback = _wanderDiagPathFallback;
    uint16 const teleportRecover = _wanderDiagTeleportRecover;
    _wanderDiagPathFallback = 0;
    _wanderDiagTeleportRecover = 0;

    BOT_LOG_INFO("npcbots", WANDER_SAMPLE_ROW_FMT,
        WanderLogPad(me->GetName(), 14),
        me->GetEntry(),
        uint32(_botclass),
        uint32(me->GetLevel()),
        me->GetMapId(),
        state,
        fmt::format("{:.0f}%", me->GetHealthPct()),
        nodeId,
        pathFallback,
        teleportRecover,
        WanderLogPad(nodeName, 18),
        WanderLogPad(targetName, 18));
}
