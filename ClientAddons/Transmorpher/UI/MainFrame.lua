...
-- TITLE BAR
-- ============================================================
local mainFrameTitle = "|cffF5C842幻化大师|r  |cff6a6050v" .. ns.VERSION .. "|r"
...
stats:SetScript("OnEnter", function(self)
    ...
    GameTooltip:AddLine("|cffF5C842当前幻化|r", 1, 1, 1)
    if not TRANSMORPHER_DLL_LOADED then
        GameTooltip:AddLine("DLL未加载", 1, 0.3, 0.3)
    ...
    if #morphedSlots > 0 then
        GameTooltip:AddLine("|cffF5C842物品：|r " .. table.concat(morphedSlots, ", "), 0.9, 0.8, 0.5, true)
    end
    if TransmorpherCharacterState then
        ...
        if s.MountHidden then mountEntry = "|cff888888已隐藏|r" end

        if mountEntry then
            GameTooltip:AddLine("|cff80cc80坐骑：|r " .. mountEntry, 0.5, 0.8, 0.5, true)
        end

        -- 2. Morph
        if s.Morph then GameTooltip:AddLine("|cffd676ff幻化：|r 模型ID " .. s.Morph, 0.84, 0.46, 1) end
        if s.PetDisplay then GameTooltip:AddLine("|cff88ddff宠物：|r 模型ID " .. s.PetDisplay, 0.53, 0.87, 1) end
        if s.HunterPetDisplay then GameTooltip:AddLine("|cffff8844战宠：|r 模型ID " .. s.HunterPetDisplay, 1, 0.53, 0.27) end
        if s.EnchantMH then GameTooltip:AddLine("|cffffa040附魔主手：|r ID " .. s.EnchantMH, 1, 0.63, 0.25) end
        if s.EnchantOH then GameTooltip:AddLine("|cffffa040附魔副手：|r ID " .. s.EnchantOH, 1, 0.63, 0.25) end
        if s.TitleID then GameTooltip:AddLine("|cffcccc44称号：|r ID " .. s.TitleID, 0.8, 0.8, 0.27) end
    end
    if GameTooltip:NumLines() == 1 then
        GameTooltip:AddLine("无活跃幻化", 0.5, 0.5, 0.5)
    end
    GameTooltip:Show()
end)
...
function ns.UpdateMorphStatusBar()
    ...
    if not dllOk then
        mainFrame.morphStatus:SetText("|cffff4444DLL未加载|r")
        return
    end
    ...
    if itemCount > 0 then table.insert(parts, "|cffF5C842" .. itemCount .. " 个物品|r") end

    if TransmorpherCharacterState then
        if TransmorpherCharacterState.Morph then table.insert(parts, "|cffd676ff已幻化|r") end
        if TransmorpherCharacterState.MountDisplay or TransmorpherCharacterState.MountHidden then
            table.insert(parts, "|cff80cc80坐骑|r")
        end
        if TransmorpherCharacterState.PetDisplay then table.insert(parts, "|cff88ddff宠物|r") end
        if TransmorpherCharacterState.HunterPetDisplay then table.insert(parts, "|cffff8844战宠|r") end
        if TransmorpherCharacterState.EnchantMH or TransmorpherCharacterState.EnchantOH then table.insert(parts, "|cffffa040附魔|r") end
        if TransmorpherCharacterState.TitleID then table.insert(parts, "|cffcccc44称号|r") end
    end

    if #parts > 0 then
        mainFrame.morphStatus:SetText(table.concat(parts, "  |cff555555\194\183|r  "))
    else
        mainFrame.morphStatus:SetText("|cff6a6050无活跃幻化|r")
    end
end
...
-- TAB SYSTEM
-- ============================================================
local TAB_NAMES = {"预览", "配装", "坐骑", "宠物", "战宠", "幻化", "杂项", "设置"}
mainFrame.tabs = {}
