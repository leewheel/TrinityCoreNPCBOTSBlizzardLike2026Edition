...
local function populateAnalysis(content)
    local y = -10
    local c0 = createCard(content, "渲染覆盖", y, 150)
    createCardButton(c0, "重置", 78, -12, function()
        ns.ResetWorldAnalysisSettings()
    end)
    createCheckboxRow(c0, "平滑纹理", "worldRenderSmoothTextures", -42)
    ...
    local function updateSmoothBiasText(value)
        ...
        text:SetText("平滑度: " .. string.format("%.2f", value or 0))
    ...
    y = y - 170
    local c1 = createCard(content, "场景可见性", y, 352)
    createCheckboxRow(c1, "M2模型", "worldRenderM2", -42)
    createCheckboxRow(c1, "地形", "worldRenderTerrain", -68)
    createCheckboxRow(c1, "地形剔除", "worldRenderTerrainCulling", -94)
    createCheckboxRow(c1, "M2/WMO阴影链接", "worldRenderM2WmoShadow", -120)
    createCheckboxRow(c1, "WMO建筑", "worldRenderWmo", -146)
    createCheckboxRow(c1, "原生WMO光照", "worldRenderWmoLighting", -172)
    createCheckboxRow(c1, "脚印", "worldRenderFootprints", -198)
    createCheckboxRow(c1, "WMO纹理", "worldRenderWmoTextures", -224)
    createCheckboxRow(c1, "WMO传送门", "worldRenderWmoPortals", -250)
    createCheckboxRow(c1, "遮挡体", "worldRenderOccluders", -276)
    createCheckboxRow(c1, "M2距离淡出", "worldRenderM2Fade", -302)

    y = y - 372
    local c2 = createCard(content, "环境标志", y, 240)
    createCheckboxRow(c2, "地面杂物", "worldRenderGroundClutter", -42)
    createCheckboxRow(c2, "碰撞几何", "worldRenderCollision", -68)
    createCheckboxRow(c2, "液体表面", "worldRenderLiquidSurface", -94)
    createCheckboxRow(c2, "液体粒子", "worldRenderLiquidParticles", -120)
    createCheckboxRow(c2, "山脉/地平线", "worldRenderMountains", -146)
    createCheckboxRow(c2, "镜面光照", "worldRenderSpecularLighting", -172)
    createCheckboxRow(c2, "对象阴影", "worldRenderObjectShadow", -198)

    y = y - 260
    local c3 = createCard(content, "调试可视化", y, 110)
    createCheckboxRow(c3, "线框", "worldRenderWireframe", -42)
    createCheckboxRow(c3, "法线", "worldRenderNormals", -68)
