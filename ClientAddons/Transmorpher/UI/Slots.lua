...
local function slot_OnShiftLeftClick(self)
    if self.itemId then
        ...
        if link then
            SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r："..link.."（"..self.itemId.."）")
        else
            SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：该物品无法用于幻化。")
        end
    end
end
...
local function slot_OnEnter(self)
    ...
    if self.isHiddenSlot then
        ...
        GameTooltip:AddLine(" ")
        GameTooltip:AddLine("|cffFF6060已隐藏（裸体幻化）|r", 1, 0.4, 0.4)
    elseif not self.itemId then
        ...
        if not link then
            ...
            if self.loadFailed then
                GameTooltip:AddLine("物品 #"..self.itemId.."（缓存失败）", 0.9, 0.45, 0.45)
            else
                GameTooltip:AddLine("物品 #"..self.itemId.."（加载中...）", 0.6, 0.6, 0.6)
            end
        else
        ...
        if self.isMorphed then
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("|cffF5C842已幻化|r", 0.96, 0.78, 0.26)
        else
            local equippedId = ns.GetEquippedItemForSlot(self.slotName)
            if equippedId and equippedId == self.itemId then
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine("已装备（未幻化）", 0.5, 0.5, 0.5)
            else
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine("预览中", 0.6, 0.8, 1.0)
            end
        end
    end
    GameTooltip:Show()
