...
function ns.InitSpellsTab(parent)
        ...
        for i = 1, NUM_ROWS do
            ...
            if data then
                ...
                if targetId then
                    ...
                    row.assign:SetText(tName or ("法术 " .. targetId))
                    row.assign:SetTextColor(0.3, 1.0, 0.5)
                    row.actionBtn:SetText("更改")
                else
                    row.assign:SetText("")
                    row.actionBtn:SetText("选择")
                end
                row:Show()
            else
                row:Hide()
    ...
    title:SetText("|cffF5C842直接法术幻化|r")
    ...
    selSubTitle:SetText("按TAB键切换所有等级")
    ...
    searchHint:SetText("搜索数据库(50,000+ 法术)...")
    ...
    local function UpdateResultScroll()
        ...
        selStatus:SetText("结果: " .. #searchResults)
    ...
    RefreshMorphView()
    if #spellPool == 0 then
        DEFAULT_CHAT_FRAME:AddMessage("|cff00ccff[幻化]|r 正在扫描法术书...(请稍候，如果刚刚登录)")
    end
end
