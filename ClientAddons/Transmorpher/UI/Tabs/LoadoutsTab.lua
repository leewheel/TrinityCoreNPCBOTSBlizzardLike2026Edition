...
listTitle:SetText("|cffffd700已保存的配装|r")
...
newHint:SetPoint("LEFT", 6, 0); newHint:SetText("保存新配装...")
...
loadoutNameLabel:SetPoint("LEFT", prevTitleBg, "LEFT", 14, 6); loadoutNameLabel:SetText("|cff8a7d6a未选择配装|r")
...
btnUpdate:SetText("覆盖"); btnUpdate:Disable()
...
btnApplyLoadout:SetText("|cffF5C842应用配装|r"); btnApplyLoadout:Disable()
...
local function CreateSlot(name, icon, align, pY)
    ...
    s:SetScript("OnEnter", function(self)
        ...
        if self.itemId and self.itemId > 0 then
            ...
            GameTooltip:AddLine("空槽位", 0.5, 0.5, 0.5)
        end
        GameTooltip:Show()
    end)
...
local function UpdateLoadoutPreview(loadout)
    ...
    if not loadout then
        loadoutNameLabel:SetText("|cff8a7d6a选择一个配装|r")
    ...
    if loadout.isCurrent then loadoutNameLabel:SetText("|cffffd700当前已装备外观|r") else loadoutNameLabel:SetText("|cffffd700"..(loadout.name or "配装").."|r") end
    loadoutSubLabel:SetText(GetTalentBindingSummary())
    previewModel:SetUnit("player"); previewModel:Undress()
...
local function ApplyLoadout(loadout, isInitial)
    ...
    if not isInitial then
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 配装 '"..loadout.name.."' 已应用!")
    end
end
...
function BuildListFrames()
    ...
    curBtn.nameText:SetText("|cff88ff88当前已装备|r")
...
btnAdd:SetScript("OnClick", function()
    ...
    if name ~= "" and _G["TransmorpherLoadoutsAccount"] then
        ...
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 配装 '"..name.."' 已保存!")
        PlaySound("gsTitleOptionOK")
    end
end)
...
btnUpdate:SetScript("OnClick", function()
    if type(activeLookId) == "number" and _G["TransmorpherLoadoutsAccount"] then
        ...
        if saved then
            ...
            UpdateLoadoutPreview(l)
            BuildListFrames()
            SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 配装已更新!")
            PlaySound("gsTitleOptionOK")
        end
    end
end)
...
btnBindPrimary:SetScript("OnClick", function()
    ...
    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 已绑定 '"..saved.name.."' 至主天赋。")
end)
...
btnBindSecondary:SetScript("OnClick", function()
    ...
    BuildListFrames()
    UpdateLoadoutPreview(saved)
    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 已绑定 '"..saved.name.."' 至副天赋。")
end)
...
btnUnbindPrimary:SetScript("OnClick", function()
    ...
    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 已移除主天赋绑定。")
end)
...
btnUnbindSecondary:SetScript("OnClick", function()
    ...
    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 已移除副天赋绑定。")
end)
...
StaticPopupDialogs["TRANSMORPHER_IMPORT_CONFLICT"] = {
    text = "名为 '%s' 的配装已存在。\n\n选择'覆盖'以替换，或输入新名称并选择'另存为新'。",
    button1 = "覆盖",
    button2 = "取消",
    button3 = "另存为新",
    hasEditBox = true,
    OnShow = function(self, data)
        self.editBox:SetText(data.loadout.name .. " (已导入)")
    ...
    OnAlt = function(self, data)
        ...
        if newName == "" then newName = data.loadout.name .. " (已导入)" end
...
local function ImportLoadoutFromString(encoded, applyAfter)
    ...
    if not loadout then
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: |cffff0000导入失败:|r " .. (err or "未知错误"))
        return false
    end
    ...
    local function FinishImport(idx, finalLoadout)
        ...
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 已导入配装 '" .. (finalLoadout.name or "配装") .. "'。")
        PlaySound("gsTitleOptionOK")
        if applyAfter then
            if ns.IsMorpherReady() then
                ApplyLoadout(finalLoadout)
            else
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: |cffff0000DLL未加载|r — 配装已保存但未应用。")
            end
        end
    end
...
local function PrintExportStringToChat(encoded)
    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r 导出字符串（也显示在对话框中 — Ctrl+A, Ctrl+C）：")
    local chunkSize = 220
...
btnStringImportApply:SetText("导入并应用")
...
local function ShowStringDialog(mode, text)
    ...
    if mode == "export" then
        stringDialogTitle:SetText("导出配装")
        stringDialogHint:SetText("全选并复制(Ctrl+A, Ctrl+C)。")
        btnStringImport:Hide()
        btnStringImportApply:Hide()
        stringDialog:Show()
        ...
        stringDialogTitle:SetText("导入配装")
        stringDialogHint:SetText("粘贴TM1导出字符串，然后点击导入或导入并应用。")
...
local function ShowExportLoadoutDialog()
    ...
    if not encoded then
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: |cffff0000导出失败:|r " .. (err or "未知错误"))
        return
    end
