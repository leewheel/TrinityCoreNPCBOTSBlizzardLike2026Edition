...
local function BuildMountList()
    ...
    if resultCount then
        resultCount:SetText("|cff6a6050" .. #mountFilteredList .. " 坐骑|r")
    end
    ...
    for idx, entry in ipairs(mountFilteredList) do
        ...
        row:SetScript("OnEnter", function()
            ...
            local typeText = entry.mountType == "F" and "|cff6699cc飞行坐骑|r" or (entry.mountType == "B" and "|cff9988cc飞行+地面|r" or "|cff669966地面坐骑|r")
            GameTooltip:AddLine(typeText)
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("点击选择", 0.5, 0.5, 0.5); GameTooltip:Show()
        end)
...
filterContainer.buttons = {
    CreateFilterButton("全部", "ALL", 0),
    CreateFilterButton("地面", "G", 48),
    CreateFilterButton("飞行", "F", 96)
}
...
searchHint:SetPoint("LEFT", 2, 0); searchHint:SetText("搜索坐骑...")
...
local function UpdateStatusLabels()
    ...
    if not state then statusLabel:SetText("|cff6a6050坐骑: 无|r") return end
    if state.MountHidden then statusLabel:SetText("|cff888888坐骑: 已隐藏|r") return end
    ...
    statusLabel:SetText("|cffffd700坐骑:|r " .. (mName and ("|cffaaccff" .. mName .. "|r") or "|cff6a6050无|r"))
end
...
local headerName = headerFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); headerName:SetPoint("LEFT", 34, 0); headerName:SetText("|cffA08D65名称|r")
local headerType = headerFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); headerType:SetPoint("LEFT", 282, 0); headerType:SetText("|cffA08D65类型|r")
local headerID = headerFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall"); headerID:SetPoint("RIGHT", -8, 0); headerID:SetText("|cffA08D65模型ID|r")
...
btnSetMount:SetSize(140, 26); btnSetMount:SetPoint("BOTTOMLEFT", 10, 4); btnSetMount:SetText("|cffffd700设置坐骑|r"); btnSetMount:Disable()

local btnHideMount = ns.CreateGoldenButton("$parentBtnHideMount", mountTab)
btnHideMount:SetSize(120, 26); btnHideMount:SetPoint("LEFT", btnSetMount, "RIGHT", 6, 0); btnHideMount:SetText("|cff888888隐藏|r")

local btnResetMount = ns.CreateGoldenButton("$parentBtnResetMount", mountTab)
btnResetMount:SetSize(120, 26); btnResetMount:SetPoint("LEFT", btnHideMount, "RIGHT", 6, 0); btnResetMount:SetText("|cffcc6666重置|r")
...
AddButtonTooltip(btnSetMount, "设置坐骑", "将此外观应用于你的所有坐骑。")
AddButtonTooltip(btnHideMount, "隐藏坐骑", "让你的坐骑隐身。")
AddButtonTooltip(btnResetMount, "重置坐骑", "清除所有坐骑幻化分配。")
...
btnSetMount:SetScript("OnClick", function()
    if mountSelectedIdx and mountFilteredList[mountSelectedIdx] then
        ...
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: 坐骑已设置为 "..entry.name)
        PlaySound("gsTitleOptionOK")
    end
end)
