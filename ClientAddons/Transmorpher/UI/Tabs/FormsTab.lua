...
function ns.InitFormsTab(parent)
    ...
    headerDesc:SetText("为每个形态组分配一个模型")
    ...
    searchHint:SetText("按生物名称或模型ID搜索...")
    ...
    local function OpenSelector(card)
        ...
        selTitle:SetText(data and ("选择生物 - " .. data.name) or "选择生物")
        selSubTitle:SetText("输入名称或模型ID")
        searchBox:SetText("")
        searchHint:Show()
        selector:Show()
