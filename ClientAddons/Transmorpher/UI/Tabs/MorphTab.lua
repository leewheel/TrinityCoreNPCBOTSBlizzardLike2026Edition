...
do
    ...
    titleText:SetPoint("TOPLEFT", 12, yOff); titleText:SetText("|cffF5C842角色幻化|r"); yOff = yOff - 24
    local subtitleText = morphTab:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    subtitleText:SetPoint("TOPLEFT", 12, yOff); subtitleText:SetText("|cff998866改变你的角色模型。仅客户端生效。|r"); yOff = yOff - 24
    ...
    raceLabel:SetPoint("TOPLEFT", 10, yOff); raceLabel:SetText("|cffF5C842种族幻化|r"); yOff = yOff - 20
        ...
        for _, gender in ipairs({2, 3}) do
            ...
            btn:SetScript("OnClick", function()
                if ns.IsMorpherReady() then
                    ...
                    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已幻化为 "..raceName..gLabel.."（"..id.."）")
                end; PlaySound("gsTitleOptionOK")
            end)
            btn:SetScript("OnEnter", function(self) GameTooltip:SetOwner(self, "ANCHOR_RIGHT"); GameTooltip:AddLine(raceName..gLabel); GameTooltip:AddLine("模型ID："..ids[gender],1,1,1); GameTooltip:Show() end)
    ...
    customLabel:SetPoint("TOPLEFT", 10, yOff); customLabel:SetText("|cffF5C842自定义模型ID|r"); yOff = yOff - 18
    local customDesc = morphTab:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    customDesc:SetPoint("TOPLEFT", 10, yOff); customDesc:SetText("|cff998866按生物名称搜索或直接输入模型ID：|r"); yOff = yOff - 22
    ...
    editHint:SetPoint("LEFT", 2, 0); editHint:SetText("名称或模型ID...")
    ...
    btnApplyCustom:SetText("|cffF5C842应用|r")
    ...
    local function ApplyMorphFromInput()
        if selectedSearchID then
            if ns.IsMorpherReady() then
                ...
                UpdatePreviewModel(); ns.UpdateSpecialSlots()
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已幻化为 "..(selectedSearchName or "生物").."（"..selectedSearchID.."）")
            ...
            if id and id > 0 and ns.IsMorpherReady() then
                ...
                if TransmorpherCharacterState then TransmorpherCharacterState.Morph = id end
                UpdatePreviewModel(); ns.UpdateSpecialSlots()
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已幻化为模型ID "..id)
                PlaySound("gsTitleOptionOK")
            end
        end
        searchDropBg:Hide()
    ...
    sizeLabel:SetPoint("TOPLEFT", 10, yOff); sizeLabel:SetText("|cffF5C842角色大小|r"); yOff = yOff - 20
    ...
    btnApplySize:SetText("|cffF5C842应用大小|r")
    btnApplySize:SetScript("OnClick", function()
        ...
        if scale and scale > 0.1 and scale < 10.0 and ns.IsMorpherReady() then
            ...
            SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：角色缩放已设为 "..scale)
            PlaySound("gsTitleOptionOK")
        end
    end)
    yOff = yOff - 40
    ...
    favLabel:SetPoint("TOPLEFT", 10, yOff); favLabel:SetText("|cffF5C842已保存幻化|r"); yOff = yOff - 18
    local favDesc = morphTab:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    favDesc:SetPoint("TOPLEFT", 10, yOff); favDesc:SetText("|cff998866保存模型ID并命名以便快速访问。|r"); yOff = yOff - 18
    ...
    favNameHint:SetPoint("LEFT", 4, 0); favNameHint:SetText("名称")
    ...
    btnFavSave:SetSize(60, 20); btnFavSave:SetPoint("LEFT", favIdInput, "RIGHT", 8, 0); btnFavSave:SetText("|cffF5C842保存|r")
    local btnFavRemove = ns.CreateGoldenButton("$parentBtnFavRemove", morphTab)
    btnFavRemove:SetSize(70, 20); btnFavRemove:SetPoint("LEFT", btnFavSave, "RIGHT", 4, 0); btnFavRemove:SetText("移除"); btnFavRemove:Disable()
    yOff = yOff - 26
    ...
    local function BuildFavButtons()
        ...
        for idx, fav in ipairs(_G["TransmorpherMorphFavorites"]) do
            ...
            useBtn:SetSize(50, 18); useBtn:SetPoint("RIGHT", -2, 0); useBtn:SetText("|cffF5C842使用|r")
            useBtn:SetScript("OnClick", function()
                if ns.IsMorpherReady() then
                    ...
                    if TransmorpherCharacterState then TransmorpherCharacterState.Morph = id end
                    UpdatePreviewModel(); ns.UpdateSpecialSlots()
                    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已幻化为 "..fav.name.."（"..id.."）")
                end; PlaySound("gsTitleOptionOK")
            end)
            row:SetScript("OnClick", function()
    ...
    btnFavSave:SetScript("OnClick", function()
        ...
        BuildFavButtons()
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已保存幻化 '"..name.."'（ID："..id.."）")
        PlaySound("gsTitleOptionOK")
    end)
    btnFavRemove:SetScript("OnClick", function()
        if favSelectedIdx and _G["TransmorpherMorphFavorites"] then
            ...
            if fav then table.remove(_G["TransmorpherMorphFavorites"], favSelectedIdx)
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已移除 '"..fav.name.."'") end
    ...
    creaturesLabel:SetPoint("TOPLEFT", 10, yOff); creaturesLabel:SetText("|cffF5C842热门生物|r"); yOff = yOff - 20
    ...
    for i, creature in ipairs(popularCreatures) do
        ...
        btn:SetScript("OnClick", function()
            if ns.IsMorpherReady() then
                ...
                if TransmorpherCharacterState then TransmorpherCharacterState.Morph = id end
                UpdatePreviewModel(); ns.UpdateSpecialSlots()
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已幻化为 "..creature.name.."（"..id.."）")
            end; PlaySound("gsTitleOptionOK")
        end)
        btn:SetScript("OnEnter", function(self) GameTooltip:SetOwner(self, "ANCHOR_RIGHT"); GameTooltip:AddLine(creature.name); GameTooltip:AddLine("模型ID："..creature.id,1,1,1); GameTooltip:Show() end)
        btn:SetScript("OnLeave", function() GameTooltip:Hide() end)
    ...
    btnResetMorph:SetText("|cffF5C842重置角色模型|r")
    ...
    morphTab:SetScript("OnShow", function() infoLabel:SetText("|cff8a7d6a潜行模式下无法显示模型信息。|r") end)
end
