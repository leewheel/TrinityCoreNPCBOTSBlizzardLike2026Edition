...
for _, tierInfo in ipairs(ns.optimizationTierOptions or {}) do
    local cb = CreateOptCheckbox("Tier" .. tierInfo.key, "|cffF5C842" .. tierInfo.label .. "|r  " .. tierInfo.raids, "将当前副本层级加入始终激活的基础保护列表。\n\n开启：基础保护 + 本层级\n关闭：仅基础保护", tierInfo.settingKey, "PROTECTED_TIER:" .. tierInfo.key)
...
protTitle:SetPoint("TOPLEFT", 14, -14); protTitle:SetText("|cffF5C842法术保护（白名单）|r")

local protDesc = protCard:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
protDesc:SetPoint("TOPLEFT", protTitle, "BOTTOMLEFT", 0, -4); protDesc:SetText("本地白名单视图。运行时保护依赖于基础保护列表加上优化数据库中启用的副本层级法术集。"); protDesc:SetTextColor(0.7, 0.7, 0.7)

-- Search Section (Left Column)
local searchTitle = protCard:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
searchTitle:SetPoint("TOPLEFT", 14, -64); searchTitle:SetText("|cffA3A3A3搜索DBC法术|r")
...
searchHint:SetPoint("LEFT", 0, 0); searchHint:SetText("输入法术名称或ID...")
...
activeTitle:SetPoint("TOPLEFT", listBg, "TOPRIGHT", 10, 50); activeTitle:SetText("|cffA3A3A3已保护列表|r")
...
manualHint:SetPoint("LEFT", 4, 0); manualHint:SetText("输入ID..."); manualInput:SetScript("OnEditFocusGained", function() manualHint:Hide() end); manualInput:SetScript("OnEditFocusLost", function(self) if self:GetText() == "" then manualHint:Show() end end)

local btnManualAdd = ns.CreateGoldenButton(nil, manualBg)
btnManualAdd:SetPoint("LEFT", manualInput, "RIGHT", 4, 0); btnManualAdd:SetSize(40, 20); btnManualAdd:SetText("添加")
...
fileTitle:SetPoint("TOPLEFT", 14, -14); fileTitle:SetText("|cffF5C842基础保护列表|r")
...
fileDesc:SetText("管理存储在 optimizationdb/protected_spells.lua 中的始终激活的基础保护列表。在通用优化中启用层级文件可扩展此列表。")
...
fileSearchHint:SetPoint("LEFT", 0, 0); fileSearchHint:SetText("搜索当前保护列表...")
...
sideTitle:SetPoint("TOPLEFT", 10, -10); sideTitle:SetText("|cffA3A3A3添加或同步|r")
...
sideDesc:SetText("从DBC搜索新法术，或手动输入法术ID添加。")
...
addSearchHint:SetPoint("LEFT", 0, 0); addSearchHint:SetText("按名称/ID搜索DBC...")
...
searchResultsEmpty:SetText("输入至少2个字母或ID")
...
btnPrevPage:SetPoint("BOTTOMLEFT", fileListBg, "BOTTOMLEFT", 0, -28); btnPrevPage:SetSize(70, 22); btnPrevPage:SetText("上一页")

local btnNextPage = ns.CreateGoldenButton(nil, fileCard)
btnNextPage:SetPoint("LEFT", btnPrevPage, "RIGHT", 8, 0); btnNextPage:SetSize(70, 22); btnNextPage:SetText("下一页")

local btnReloadFile = ns.CreateGoldenButton(nil, fileCard)
btnReloadFile:SetParent(sidePanel)
btnReloadFile:SetPoint("BOTTOMRIGHT", -10, 10); btnReloadFile:SetSize(100, 22); btnReloadFile:SetText("重新加载")

local btnExportFile = ns.CreateGoldenButton(nil, fileCard)
btnExportFile:SetParent(sidePanel)
btnExportFile:SetPoint("RIGHT", btnReloadFile, "LEFT", -8, 0); btnExportFile:SetSize(100, 22); btnExportFile:SetText("导出列表")
    ...
    if ns.IsMorpherReady() then
        ...
        SimpleTimer_After(0.1, function()
            if TRANSMORPHER_PROTECTED_SAVE_OK == false then
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：导出 optimizationdb/protected_spells.lua 失败。")
            else
                SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：已导出当前保护列表到 optimizationdb/protected_spells.lua。")
            end
        end)
    end
    PlaySound("gsTitleOptionOK")
...
timeTitle:SetPoint("TOPLEFT", 12, -12); timeTitle:SetText("|cffF5C842时间控制|r")

local timeDesc = timeCard:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
timeDesc:SetPoint("TOPLEFT", timeTitle, "BOTTOMLEFT", 0, -4); timeDesc:SetText("覆盖客户端当前时间。"); timeDesc:SetTextColor(0.7, 0.7, 0.7)
...
local sliderText = _G[slider:GetName().."Text"]; sliderText:SetTextColor(1, 0.82, 0)
...
btnApplyTime:SetPoint("LEFT", slider, "RIGHT", 12, 0); btnApplyTime:SetSize(86, 24); btnApplyTime:SetText("设定时间")
btnApplyTime:SetScript("OnClick", function()
    ...
    if ns.IsMorpherReady() then
        ...
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：时间已更新。")
    end; PlaySound("gsTitleOptionOK")
end)

local btnResetTime = ns.CreateGoldenButton("$parentResetTime", timeCard)
btnResetTime:SetPoint("TOPRIGHT", timeCard, "TOPRIGHT", -12, -10); btnResetTime:SetSize(82, 20); btnResetTime:SetText("重置")
btnResetTime:SetScript("OnClick", function()
    if ns.IsMorpherReady() then
        ...
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：时间已重置为服务器默认。")
    end; PlaySound("gsTitleOptionOK")
end)
...
local atmosphereCard = CreateEnvCard(atmospherePanel, "|cffF5C842绘制距离|r", "在此控制远距离裁剪和雾效，布局更为简洁。", -8, 188)
local fogCard = CreateEnvCard(atmospherePanel, "|cffF5C842雾效覆盖|r", "从氛围标签应用自定义世界雾效。这些设置通过独立的环境配置路径同步。", -212, 208)
...
local fogEnable = ns.CreateCheckbox(fogCard, "启用雾效覆盖", "覆盖世界雾的颜色和起止距离")
...
local farClipEnable = ns.CreateCheckbox(atmosphereCard, "启用远距离裁剪覆盖", "为当前环境配置应用自定义绘制距离")
...
farClipHint:SetText("数值越大，视野越远。调整滑块会自动启用覆盖。")
...
_G[farClipSlider:GetName() .. "Text"]:SetText("远距离裁剪")

local function SyncFarClipSliderText(value)
    local text = _G[farClipSlider:GetName() .. "Text"]
    text:SetText("远距离裁剪：" .. math.floor((value or 0) + 0.5))
    text:SetTextColor(1, 0.82, 0)
end
...
local hdCard = CreateEnvCard(hdFontPanel, "|cffF5C842高清字体渲染|r", "启用类似正式服的MSDF字体渲染以获得更清晰的UI文字。此设置仅在下次启动客户端时生效。", -8, 168)

local hdModeLabel = hdCard:CreateFontString(nil, "OVERLAY", "GameFontNormal")
hdModeLabel:SetPoint("TOPLEFT", hdCard, "TOPLEFT", 12, -58)
hdModeLabel:SetText("下次启动")
...
local hdToggle = ns.CreateCheckbox(hdCard, "启用MSDF字体", "为下次客户端启动持久化MSDF字体渲染模式")
...
local function UpdateHdFontModeUI(enabled)
    ...
    hdHint:SetText(enabled and "MSDF字体渲染已排队，将在下次启动时应用。当前会话将继续使用现有字体渲染器。" or "原生字体渲染已排队，将在下次启动时应用。当前会话将继续使用现有字体渲染器。")
end

StaticPopupDialogs["TRANSMORPHER_MSDF_RESTART"] = {
    text = "MSDF字体变更仅在关闭并重新打开客户端后生效。",
    button1 = OKAY,
    timeout = 0,
    whileDead = true,
    hideOnEscape = true,
...
hdToggle:SetScript("OnClick", function(self)
    ...
    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r: MSDF字体渲染已排队，将在下次启动客户端时生效。")
...
titleSearchHint:SetPoint("LEFT", 0, 0); titleSearchHint:SetText("搜索称号...")
...
btnResetTitle:SetPoint("RIGHT", -8, 0); btnResetTitle:SetSize(76, 22); btnResetTitle:SetText("重置")
btnResetTitle:SetScript("OnClick", function()
    if ns.IsMorpherReady() then
        ...
        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：称号已重置为原始。")
        PlaySound("gsTitleOptionOK")
    end
end)
...
local function UpdateTitles()
            ...
            if query == "" or name:lower():find(query, 1, true) then
                ...
                if not b then
                    ...
                    b:SetScript("OnClick", function(self)
                        ...
                        SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：称号已设为："..self.titleName)
                        PlaySound("gsTitleOptionOK")
                    end)
                    titleBtns[y] = b
                end
    ...
    if titleResultCount then
        if y > 0 then titleResultCount:SetText("|cffC8AA6E" .. y .. " 个称号|r")
        else titleResultCount:SetText("|cff6a6050未找到称号|r") end
    end
end
