...
local function CreateEyeButton(slot, isSpecial)
    ...
    eyeBtn.UpdateTooltip = function(self)
        ...
        if self.isHidden then
            GameTooltip:SetText("|T" .. TEX_RESTORE .. ":16:16|t " .. self.slotName .. "：|cffd676ff已隐藏|r")
            GameTooltip:AddLine("点击显示此槽位", 0.7, 0.7, 0.7)
        else
            GameTooltip:SetText("|T" .. TEX_HIDE .. ":16:16|t " .. self.slotName .. "：|cff6bc7ff可见|r")
            GameTooltip:AddLine("点击隐藏此槽位", 0.7, 0.7, 0.7)
        end
        GameTooltip:Show()
    end
