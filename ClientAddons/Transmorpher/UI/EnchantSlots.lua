...
for _, eName in ipairs(ns.enchantSlotNames) do
        ...
        if button == "LeftButton" then
            if IsAltKeyDown() and self.enchantId then
                if ns.IsMorpherReady() then
                    ...
                    self.isMorphed = true
                    SELECTED_CHAT_FRAME:AddMessage("|cffF5C842<幻化>|r：附魔已应用到"..(enchantDisplayName[eName] or eName).."！")
    ...
    eSlot:SetScript("OnEnter", function(self)
        ...
        if self.enchantId then
            GameTooltip:AddLine("|cffF5C842"..(self.enchantName or "附魔").."|r")
            GameTooltip:AddLine("附魔ID: "..self.enchantId, 0.7, 0.7, 0.7)
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("Alt+点击应用附魔幻化", 0.5, 0.8, 0.5)
            GameTooltip:AddLine("右键点击移除", 0.8, 0.5, 0.5)
        else
            GameTooltip:AddLine(enchantDisplayName[eName] or eName)
            GameTooltip:AddLine("点击浏览附魔效果", 0.7, 0.7, 0.7)
        end
        GameTooltip:Show()
    end)
